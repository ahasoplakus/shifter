...
The workflow below shows a step-by-step programming approach to create Laboratory Shift Tables using the `{tidyverse}` suite for data wrangling, and `{gt}` to create the desired Shift Table layout.
...
-   Subset the resulting data for safety subjects where analysis flag is equal to `Y` and at post-dose
...
Create a dummy dataset that contains a row for each `BNRIND` and corresponding `ANRIND` by Treatment.
...
An alternate and tidier approach would be to create a function say `build_shift_table()` to cover <b>Steps 3-5</b>
