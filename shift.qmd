...
The workflow below shows a step-by-step programming approach to create Shift Tables for Laboratory recrds using the `{tidyverse}` suite for data processing, and `{gt}` to create the desired Shift Table layout.
...
out <-
  ...
  tab_header(
    title = md("Protocol: xxxx<br><br>Table x.x<br>Shift Table of Lab Hematology<br>(Full Safety Analysis Set)"),
