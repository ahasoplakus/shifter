...
# some user defined functions are in paths below
...
flag is equal to `'Y'`
-   Subset data to keep lab records within the desired time period
(eg. `Week 2-Week 6`)
...
glimpse_dataset(
  adlb_hgb,
  exprs(USUBJID, TRT01A, TRT_VAR, PARCAT1, PARAM, AVISIT, BNRIND, ANRIND, AVAL)
)
```
...
`count_shifts()` to cover <b>Steps 3-5</b>
...
shift_final <- shift_final |>
  ...
  bind_cols(num_to_pct(shift_final, trt_bign))
```
...
### 7. Displaying Final Table with `{gt}`
